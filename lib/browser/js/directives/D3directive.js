...
app.directive('treeVisual', ['$compile', '$rootScope', function ($compile, $rootScope) {
	function link(scope, element, attrs) {
		...
		treeJSON = d3.json("data", function (error, treeData) {
			...
			//HERE WE ADD THE COLLAPSE BUTTON.
			...
			d3.select(element[0]).select('#collapseBtn')
				.on("click", function () {
					//Sort by ID so that it always shrinks into the root node
					svg.selectAll(".node").sort(function (a, b) { return a.id - b.id })
						.each(function (d, i) {
							if (d.children) {
			...
			function update(source) {
				...
				//This must be done because SVG does not believe in line breaks (yet)
				...
				var nodeTypeG = nodeEnter.append('g');
				nodeTypeG.append("rect")
					.attr("x", function (d) { return d.children || d._children ? -13 : 13; })
					.attr("dy", "-.35em")
					.attr("width", function (d) { return this.parentNode.getBBox().width + 30; })
					.attr("height", function (d) { return 40; })
					.style("fill", "#7bdf00");
				nodeTypeG.append('text')
					.attr("class", "nodeType")
					.attr("dy", "-.35em")
					.style("font-size", "12")
					.style("fill", "##ffffff").
